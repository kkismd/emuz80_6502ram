...
- 負の数のサポート
  - オリジナルのVTLC02は数値の範囲が 0～65535 から -32768～32767 に変更になりました。
...
- POKE/PEEKの構文変更
  ...
  - 16ビット値を一度に書き込むPOKE16文を追加しました。
  - コマンド文字は`@`の代わりに`<`を使い、`<=書き込み先アドレス,書き込む値`という形になります。
...
- 左右シフト演算子
  - 元のVTLC02では`THEN`演算子と`ELSE`演算子に使われていた `[`と`]`をそれぞれ左シフト、右シフトに変更しました。
